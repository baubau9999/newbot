...
message.react('887650847327158343');
					const embed = new MessageEmbed()
						.setDescription(`[\`📄 View Lịch Sử Tin Nhắn\`](${response.url})`)
						.setColor('GREEN');
					message.reply('Đã Lưu Lại Thành Công', embed);
          await message.channel.send(embed).then(embed => { embed.delete({ timeout: 2000 }) })
				});
			}
		}
		else {
